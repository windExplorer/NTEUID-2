...
指令列表：
  添加刮刮乐cookie <cookie>  — 私聊绑定 kf cookie，自动抓取数据
  更新刮刮乐                 — 刷新刮刮乐数据
  刮刮乐统计                 — 查看累计统计
  今日刮刮乐                 — 查看今日刮刮乐数据
"""
...
sv_scratch_bind = SV("nte刮刮乐绑定", area="DIRECT")
...
async def nte_scratch_bind(bot: Bot, ev: Event):
    ...
    if not cookie:
        return await bot.send(COOKIE_HELP)
    # 简单的 cookie 格式校验
    if "=" not in cookie:
        return await bot.send(f"cookie 格式似乎不对，请检查。\n{COOKIE_HELP}")
